...
class Raven {
    ...
    update(deltaTime) {
        ...
        if (this.timeSinceFlap > this.flapInterval) {
            ...
            this.timeSinceFlap = 0;
            if (this.hasTrail){
                for(let i=0; i<5; i++) particles.push(new Particle(this.x, this.y, this.width, this.color))
            } 
...
class Explosion {
    ...
    update(deltaTime){
        this.timeSinceLastFlame += deltaTime;
        if (this.timeSinceLastFlame > this.frameInterval) {
            this.frame++;
            this.timeSinceLastFlame = 0;
...
window.addEventListener('click', (e)=>{
    const detetectPixelColor = collisionCtx.getImageData(e.x, e.y, 1, 1)
    
    const pc = detetectPixelColor.data
